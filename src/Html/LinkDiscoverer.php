...
final class LinkDiscoverer
{
    ...
    /**
     * @param list<string> $excludePatterns Regex patterns for URLs to skip
     *
     * @return list<DiscoveredHref>
     */
    public function discover(string $html, string $sourceUrl, array $excludePatterns = []): array
    {
        if (trim($html) === '') {
            return [];
        }
        ...
        foreach ($crawler->filter('a[href]') as $element) {
            ...
            $discovered[] = new DiscoveredHref(url: $cleanUrl, anchorText: $anchorText, isExternal: $isExternal);
        }
